...
def parse_args():
    ...
    parser.add_argument('--train-data-path', type=str, required=True, help="Path to the train data file.")
...
args = parse_args()
print(args)
task_name = os.path.basename(args.train_data_path).split('.')[0].replace('_', '-')
...
print("Loading data......")
roi_features, adj_mats, labels = utils.load_data(args.train_data_path,args.lamb)
print(roi_features.shape, adj_mats.shape, labels.shape)

test_roi, test_adj_mats, test_labels = utils.load_data(args.test_data_path,args.lamb)

test_dataset = Data.TensorDataset(test_roi, test_adj_mats, test_labels)
...
ACClist, SENlist, SPElist, BAClist, F1list, MCClist, AUClist = [], [], [], [], [], [], []

for fold, (train_index, val_index) in enumerate(kfold.split(roi_features, labels)):
    ...
    train_roi_features = roi_features[train_index]
    train_labels = labels[train_index]
    train_adj = adj_mats[train_index]

    val_roi_features = roi_features[val_index]
    val_labels = labels[val_index]
    val_adj = adj_mats[val_index]
    ...
    best_val_acc = 0.0
    best_model_path = f"{fold + 1}-{task_name}-OC-best.pth"
    for epoch in range(1, args.epochs + 1):
        ...
        for step, (train_roi_features, train_adjs, train_labels) in enumerate(train_loader):
            ...
            all_outputs.append(output.cpu())  # Collect outputs
            all_labels.append(train_labels.cpu())  # Collect labels
        ...
        with torch.no_grad():
            for step, (val_roi_features, val_adjs, val_labels) in enumerate(val_loader):
                ...
                all_val_outputs.append(output_val.cpu())  # Collect validation outputs
                all_val_labels.append(val_labels.cpu())  # Collect validation labels
    ...
    print(f'Fold {fold + 1} Test | acc {test_ACC * 100:.2f} | sen {test_SEN * 100:.2f} | spe {test_SPE * 100:.2f} | bac {test_BAC * 100:.2f} | F1 {test_F1 * 100:.2f} | mcc {test_MCC * 100:.2f} | auc {test_AUC * 100:.2f}')
